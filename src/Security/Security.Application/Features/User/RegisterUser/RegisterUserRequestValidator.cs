...
public class RegisterUserRequestValidator : AbstractValidator<RegisterUserRequest>
{
    public RegisterUserRequestValidator()
    {
        ...
        RuleFor(f => f.Password).NotEmpty().WithMessage("RegisterUserRequest.Password can't be null")
            .MinimumLength(8).WithMessage("RegisterUserRequest.Password length can't be less than 8");
        RuleFor(f => f.PasswordConfirm).NotEmpty().WithMessage("RegisterUserRequest.PasswordConfirm can't be null")
            .MinimumLength(8).WithMessage("RegisterUserRequest.PasswordConfirm length can't be less than 8");
